...
class Autoencoder:
    ...
    def unroll_vae(self, latent_space_dim):
        '''
            Unrolls the pretrained RBM network into a DFF keras model
            and sets hidden layer parameters to pretrained values.
        ...
        for i in range(self.num_hidden_layers):
            weights = [self.W[i],self.b[i].flatten()]
            if (i == self.num_hidden_layers - 1):
                x = Dense(self.layer_dims[i+1],weights = weights)(x)
            else:
                x = Dense(self.layer_dims[i+1],
                          activation='sigmoid',weights = weights)(x)

        # Build bottleneck

        _shape_before_bottleneck = K.int_shape(x)[1:]
        x = Flatten()(x)

        self.mu = Dense(latent_space_dim, name="mu")(x)
        self.log_variance = Dense(latent_space_dim,
                                  name="log_variance")(x)
